...
var dataLoaded = function (json, isInit) {
	...
	if (SITE_INFO.blocktids && TOOLS.in_array(json.Variables.thread.tid, SITE_INFO.blocktids)) {
		TOOLS.hideLoading();
		TOOLS.dialog({content: "帖子已關閉", noMask: true, autoClose: 1});
		return;
	}
	if (isInit) {
		...
		threadTitle = json.Variables.thread.subject ? json.Variables.thread.subject : '快來看看這個話題';
	...
	if (isshow) {
		...
		if (!showinit) {
			...
			$('.replyBtn').html('<i class="incoR"></i>參與');
...
var recommend = function (obj) {
	if (member_uid == "0") {
		TOOLS.showTips('您尚未登錄，沒有權限點贊', true);
