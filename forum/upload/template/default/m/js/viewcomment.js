...
var bindCommentEvent = function () {
	$('a.replyBtn').unbind("click").click(function () {
		if (member_uid != "0") {
			replyComment(formhash, tId, pId, '樓層', 'commentsubmit=yes&comment', 1, 0, 0);
		}
	});
};
...
var dataLoaded = function (json, isInit) {
	...
	if (isInit) {
		...
		threadTitle = json.Variables.thread.subject ? json.Variables.thread.subject : '快來看看這個曬圖';
		threadContent = '我參加了#' + SITE_INFO.siteName + '#舉辦的曬圖活動，快來幫我點贊吧';

		if (postItem.authorid == member_uid) {
			$('.tipBor').html('希望更多人喜歡你的照片，點擊右上角圖標分享<span class="arrLeft"></span>');
		} else {
			$('.tipBor').html('如果喜歡我的照片，請點擊右上角圖標分享<span class="arrLeft"></span>');
		}
